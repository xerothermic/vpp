...
static u8 *format_patlu_trace (u8 *s, va_list *args)
{
  ...
  // s = format (s, "PATLU: ipv4 proto %d, udp [src,dst] port [%d,%d], QR=[%x] dns name: %s",
  //             t->ipv4_protocol, t->udp_src_port, t->udp_dst_port, t->flags_qr, t->dns_request_name);
  if (t->is_dns_reply) {
    s = format (s, "PATLU: %U requests %s -> %U",
              format_ip4_address, &t->dns_client_ip, t->dns_request_name,
              format_ip4_address, &t->dns_resp_ip);
    FILE * fp = fopen("/tmp/dns.log", "a");
    fformat (fp, "PATLU: %U requests %s -> %U\n",
              format_ip4_address, &t->dns_client_ip, t->dns_request_name,
              format_ip4_address, &t->dns_resp_ip);
    fclose(fp);
  }
...
_ (QUERY, "DNS query packets processed")  \
...
static inline void handle (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b0,
                            u32* query_pkts, u32* reply_pkts, u32* other_pkts)
{
  ...
  u16 flags_qr = -1;
  /**
   *  XXX: Not sure what will happen if we keep the buffer pointer shifted?
   *        Better to shift back now.
   */
  vlib_buffer_advance(b0, -sizeof (ethernet_header_t));

  if (ip40->protocol == 17 && clib_net_to_host_u16 (u0->src_port) == 53) {
    u16 flags = clib_net_to_host_u16(d0->flags);
    flags_qr = flags & DNS_QR;
    if (flags_qr) {
      (*reply_pkts)++;
      is_dns_reply = true;
    }
  ...
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE) &&
                     (b0->flags & VLIB_BUFFER_IS_TRACED)))
    {
      ...
      if (PREDICT_FALSE (is_dns_reply)) {
        ...
        t->dns_request_name = 0;
        t->flags_qr = flags_qr;
        t->ipv4_protocol = ip40->protocol;
        t->udp_src_port = clib_net_to_host_u16 (u0->src_port);
        t->udp_dst_port = clib_net_to_host_u16 (u0->dst_port);
        ...
        {
          u8 *name0 = (*fp) (label0, (u8 *)d0, (u8 **)&q0);
          // needs to make name0 null-terminated.
          t->dns_request_name = format (0, "%s%c", name0, 0);
        }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
        {
          ...
          handle(vm, node, b0, &dns_query_pkts, &dns_reply_pkts, &other_pkts);
          handle(vm, node, b1, &dns_query_pkts, &dns_reply_pkts, &other_pkts);
  ...
  vlib_node_increment_counter (vm, patlu_node.index, PATLU_ERROR_QUERY,
                               dns_query_pkts);
